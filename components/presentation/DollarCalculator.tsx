      ...
      <div className="relative flex h-full flex-1 flex-col">
          ...
          {p.checks.map((c) => {
            ...
            return (
              <li key={c} className="flex items-start gap-2.5">
                <Check size={16} strokeWidth={2.5} className={`mt-[3px] shrink-0 ${T.accent}`} />
                <span className={`text-[15px] leading-snug ${esHerencia ? `font-bold ${T.primary}` : `font-medium ${T.secondary}`}`}>
                  {esHerencia ? `${c}, más:` : c}
                </span>
              </li>
            );
        ...
        <div className={`mt-auto border-t pt-5 ${T.border}`}>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex w-full items-center justify-center gap-2 rounded-full bg-[var(--color-accent)] py-3 text-[13px] font-bold uppercase tracking-widest text-white"
          >
            Inscribirme ahora
          </a>
