 ...
 */
const PLANES: Record<string, { nombre: string; monto: string; snippet: string; tier: "base" | "mid" | "vip" }> = {
  junior: {
...
const tierBadge: Record<"base" | "mid" | "vip", { texto: string; icon: typeof Crown } | null> = {
  base: null,
  mid: { texto: "Especialización", icon: Star },
  vip: { texto: "Ruta profesional completa", icon: Crown },
};
...
export default async function PagarPage({ params }: { params: Promise<{ plan: string }> }) {
  ...
  if (!data) notFound();
  const badge = tierBadge[data.tier];

  return (
    <div className="flex min-h-screen items-center justify-center bg-[#0b0c0e] px-6 py-16">
      <div
        className={`relative w-full max-w-sm rounded-2xl border px-8 py-10 text-center shadow-[0_30px_60px_-30px_rgba(0,0,0,0.6)] ${
          data.tier === "vip" ? "border-[#b8935a]/50 bg-[#b8935a]/[0.06]" : "border-white/10 bg-white/[0.03]"
        }`}
      >
        {badge && (
          <span className="absolute -top-3.5 right-6 flex items-center gap-1.5 rounded-full bg-[#b8935a] px-3.5 py-1.5 text-[10.5px] font-bold uppercase tracking-wide text-[#0b0c0e]">
            <badge.icon size={12} strokeWidth={2.5} /> {badge.texto}
          </span>
        )}
